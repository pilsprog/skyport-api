...
public class Map {
    ...
    public BFSIterator iterator(Point from) {
        return new BFSIterator(this, from);
    }
    ...
    public Tile getData(Point p) {
        if (p.getJ() < 0 || p.getJ() >= jLength || p.getK() < 0 || p.getK() >= kLength) {
            return Tile.VOID;
        }
    ...
    public List<Point> neighbours(Point p) {
        List<Point> points = Arrays.asList(new Point(p.getJ() + 1, p.getK() + 1), // down
